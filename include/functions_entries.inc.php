...
function serendipity_printEntries($entries, $extended = 0, $preview = false, $smarty_block = 'ENTRIES', $smarty_fetch = true, $use_hooks = true, $use_footer = true, $use_grouped_array = false) {
    ...
    // Bypass the cache when previewing a comment (the preview comment is added from POST data
    // and must not be served from cache) or when returning from a comment submission
    // (the is_comment_added / is_comment_moderate flags must be rendered fresh).
    $bypass_cache = !empty($serendipity['POST']['preview']) || !empty($serendipity['GET']['csuccess']);
