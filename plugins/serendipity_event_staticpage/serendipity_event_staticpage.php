        ...
        switch($spcat) {
            ...
            default:
                ...
                if (!empty($serendipity['POST']['staticPreview'])) {
                    ...
                    // 1. showForm re-renders the form with the user's current edits (not DB values)
                    // 2. The session data below reflects the unsaved edits
                    $bag = new serendipity_property_bag;
                    $this->introspect($bag);
                    $config_names = $bag->get('page_configuration');
                    foreach ($config_names as $config_item) {
                    ...
                    echo '</script>';
                    $serendipity['POST']['staticSubmit'] = true;
                    echo '<span class="msg_notice"><span class="icon-info-circled" aria-hidden="true"></span> ' . sprintf(PLUGIN_STATICPAGE_PREVIEW, '<a href="' . $link . '">' . (function_exists('serendipity_specialchars') ? serendipity_specialchars($this->staticpage['pagetitle']) : htmlspecialchars($this->staticpage['pagetitle'], ENT_COMPAT, LANG_CHARSET)) . '</a>') . '</span>';
                }
